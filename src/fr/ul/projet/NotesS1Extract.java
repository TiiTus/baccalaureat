...
/**
 * Classe pour gérer l'extraction des notes de la première session des candidats
 * selon leur profil
 * 	- la série
 * 	- la fillière
 * 	- la spécialité
 ...
 *
 */
public class NotesS1Extract {

	private static final Logger LOG = Logger.getLogger(EpreuveImport.class.getName());


	//constructeur
	public NotesS1Extract() {
		super();
	}

	/**
	 * extraction des notes de la première session
	 * @param serie
	 * @param filliere
	 * @param specialite
	 * @param section
	...
	private boolean export(String serie, String filliere, String specialite, String section) {
		boolean res = true;
		String sql = "";
		String filename = "../samples/export/notes_session1.csv";
		...
		try {
			...
			try {
	            ...
	            fw.flush();
	            fw.close();
	            System.out.println("Données enregistrées dans /samples/export/notes_session1.csv");
	...
	public static void main(String[] args) {
		...
		//traitement
		NotesS1Extract ns1e = new NotesS1Extract();
		DBManager.URI= dburi;
		...
		ns1e.updateDB(serie, filliere, spe, section);
	}
