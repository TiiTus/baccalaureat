...
/**
 * Classe qui gère les imports des données dans la table Preciser qui fait le lien entre une matiere et une epreuve
 * @author hansjulien
 *
 */
public class PreciserImport {
	...
	/**
	 * insérer ou les données dans la table Preciser
	 * @param idMat
	 * @param idEp
	 * @return res (boolean)
	 */
	private boolean add(int idMat, String idEp) {
		boolean res = true;
		String sql = "";
		PreparedStatement ps = null;
		sql = "INSERT into preciser(Matiere_idMatiere,Epreuve_idEpreuve)"
				+" VALUES(?,?)";
		try {
	...
	public int updateDB(CSVParser parser) {
		...
		for (CSVRecord item : parser) {
			...
			String idMatiere1 = item.get(2).trim();
			int idMatiere = Integer.parseInt(idMatiere1);

			//enregistrer
			if (add(idMatiere, idEpreuve)){
				res++;
			}
		}
		DBManager.quit();
	...
	public static void main(String[] args) {
		...
		//traitement
		PreciserImport cl = new PreciserImport(filename);
		DBManager.URI= dburi;
