...
/**
 * Classe qui gère l'import des epreuves composées dans la table Composer
 * @author hansjulien
 *
 */
public class ComposerImport {
	...
	/**
	 * insérer les epreuves composées
	 * @param idEp1
	 * @param idEp2
	 * @return res (boolean)
	 */
	private boolean add(String idEp1, String idEp2) {
		...
		sql = "INSERT into composer(Epreuve_idEpreuve1,Epreuve_idEpreuve2)"
				+" VALUES(?,?)";
		try {
	...
	public int updateDB(CSVParser parser) {
		...
		for (CSVRecord item : parser) {
			...
			String idEpreuve2 = item.get(2).trim();
	...
	public static void main(String[] args) {
		...
		//traitement
		ComposerImport cl = new ComposerImport(filename);
		DBManager.URI= dburi;
